...
const useCloudList = (): {
  ...
  const fetchCloudList = async (current_page: number): Promise<void> => {
    try {
      const { data } = await axios.get<CloudData>(
        `${API_BASE_URL}/cloud/list?current_page=${current_page}&page_size=10`
      );
      if (data) {
        setCloudList(data);
      }
  ...
  const filterCloudList = async (
    current_page: number,
    filter_entity: CloudFiltersDataEntity
  ): Promise<void> => {
